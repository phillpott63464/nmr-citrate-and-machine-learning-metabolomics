...
referenceData = pd.DataFrame({'multiplets': [pd.DataFrame({'center': 0.0, 'scale': 1.0, 'ssm_indices': [[0, 1, 2, 3, 4, 5, 6, 7, 8]]}), None, None],
...
metabolites = pd.read_csv(dirPath.joinpath('Casmdb_Data/metabolites.csv'), index_col=0)
...
multiplets = pd.read_csv(dirPath.joinpath('Casmdb_Data/multiplets.csv'), index_col=0)
couplings = pd.read_csv(dirPath.joinpath('Casmdb_Data/couplings.csv'), index_col=0)


def createTrainingData(substanceSpectrumIds=[], sampleNumber=1, frequency=600.00, points=2**15, limits=(10, -2),
                       highLimits=(9, 11), lowLimits=(-1, -3), peakWidth=1.5, scale=1.0,
                       rondomlyScaleSubstances=True, includeNoise=True, noiseRange=(-2,-3), referenceSubstanceSpectrumId='tsp',
                       randomlyOffsetMultiplets=True, multipletOffsetCap = 0.01):
    """
        ...
        """
    scalesDict = {ssid: [scale * (10 ** random.uniform(0.0, 1.0)) if rondomlyScaleSubstances else scale for samNum in range(sampleNumber)] for ssid in substanceSpectrumIds + [referenceSubstanceSpectrumId]}
    ...
    for sampleNumber in tqdm(range(sampleNumber)):
        ...
        positions, y = generateSignal(referenceData.loc[referenceSubstanceSpectrumId, 'ssm'], peakWidth, frequency, points, limits, scalesDict[referenceSubstanceSpectrumId][sampleNumber])
        for spectrumId in substanceSpectrumIds:
            if sampleNumber == 0:
                ssm = getSsmData(spectrumId=spectrumId, referenceOffset=referenceData.loc[referenceSubstanceSpectrumId, 'offset'], transform=False, multipletOffsetCap=multipletOffsetCap)
                x, substanceY = generateSignal(ssm, peakWidth, frequency, points, limits, scalesDict[spectrumId][sampleNumber])
                untransformedComponentsList.append(substanceY)
            ssm = getSsmData(spectrumId=spectrumId, referenceOffset=referenceData.loc[referenceSubstanceSpectrumId, 'offset'], transform=randomlyOffsetMultiplets, multipletOffsetCap=multipletOffsetCap)
            x, substanceY = generateSignal(ssm, peakWidth, frequency, points, limits, scalesDict[spectrumId][sampleNumber])
    ...
    return pd.DataFrame(scalesDict), positions, np.vstack(intensitiesList), np.vstack(untransformedComponentsList)
...
def getSsmData(spectrumId, referenceOffset, transform, multipletOffsetCap):
    ...
    for index, row in couplingsData.iterrows():
        ssm[multipletsData.index.get_loc(row['multiplet_id1']), multipletsData.index.get_loc(row['multiplet_id2'])] = float(row['j'])
    ssm = ssm + ssm.T
    for index, center in enumerate(multipletsData['center']):
        chemShift = float(center) + referenceOffset + random.uniform(-multipletOffsetCap, multipletOffsetCap) if transform else float(center) + referenceOffset
