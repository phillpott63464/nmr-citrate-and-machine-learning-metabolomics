...
sampleNumber = 3
scales, positions, intensities, components = createTrainingData(substanceSpectrumIds=substanceSpectrumIds, sampleNumber=sampleNumber)
...
np.savetxt(outDir.joinpath('components.csv'), np.transpose(components), delimiter=',', header=','.join([str(aminoAcid) for aminoAcid in aminoAcids]))
