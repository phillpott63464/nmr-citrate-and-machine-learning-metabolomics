...
def _(
    ...
    print(
        f'Held-back metabolites: {held_back_metabolites}, unused for this model'
    )
...
def _(
    ...
    def get_training_data_mlp(
        ...
        # substanceDictnew = substanceDict['Citric acid'] # Only look for citric acid
        ca_key = substanceDict['Citric acid'][0]
        print(ca_key)
        reference_spectra = {ca_key: reference_spectra[ca_key]}
        ...
        with h5py.File(file_path, 'w') as f:
            ...
            val_idx = 0
            for i, spec_idx in tqdm(
                enumerate(val_indices), total=len(val_indices)
            ):
                if i in train_indices:
                    for substance in reference_spectra:
                        spectrum = spectra.__getitem__(
            ...
            test_idx = 0
            for i, spec_idx in tqdm(
                enumerate(test_indices), total=len(test_indices)
            ):
                if i in train_indices:
                    for substance in reference_spectra:
                        spectrum = spectra.__getitem__(
