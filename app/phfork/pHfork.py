...
class AcidAq:
    ...
    def __init__(self, Ka=None, pKa=None, charge=None, conc=None, name=None):
        ...
        self._Ka_temp = np.append(1.0, self.Ka)
...
ionic strength: {0.I:.3e} M\n\n"""
    ...
    header_line = (
        f"{'Species':15}{'Charge':10}{'Ka':15}{'pKa':10}{'Conc. / M':15}\n"
    )
    ...
    def __str__(
        ...
        for cpd in self.species:
            ...
            if isinstance(cpd, AcidAq):
                ...
                for charge, ka, pka, conc in props:
                    if ka is not np.nan:
                        prt_str += self.acid_line.format(
                            name, charge, ka, pka, conc
                        )
                    else:
        ...
        if hasattr(self, 'pH'):
            ...
            possible_h3o = np.roots([1.0, charge_conc, -self.Kw])
    ...
    def pHsolve(
        ...
        self.pHsolution = spo.root_scalar(
            self._diff_pos_neg,
            bracket=bracket,
            method=method,
            xtol=xtol,
