...
def add_lorentzians(x, peaklist):
    # Convert peaklist to numpy array for vectorized ops
    ...
    hw2 = (0.5 * w) ** 2  # shape (N, 1)

    x = x[np.newaxis, :]  # shape (1, M) for broadcasting

    # Compute the Lorentzian for all peaks and all x
    y = I * (hw2 / (hw2 + (x - v0) ** 2))  # shape (N, M)

    return np.sum(y, axis=0)  # shape (M,)


def add_gaussians(x, peaklist):
    wf = 0.4246609
